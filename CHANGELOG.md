...
- **observables (GLRX-13: Pseudorange computation pipeline)**
  - Реализовано вычисление псевдодальности на основе code phase + TOW:
    - `t_tx = tow_s + code_phase_chips / chip_freq_hz`
    - восстановление времени передачи сигнала
  - Добавлена коррекция спутниковых часов на основе эфемерид:
    - af0 / af1 / af2
    - релятивистская поправка орбиты
  - Добавлена ионосферная коррекция (Klobuchar model)
  - Добавлена тропосферная коррекция (упрощённая модель Саастамойнена)
  - Реализовано вычисление доплера и псевдоскорости на основе carrier tracking
  - Добавлена оценка C/N₀ и весов WLS:
    - NW-PR based estimator
    - порог пригодности сигнала: 25 dB-Hz
  - Введена единая структура наблюдаемого:
    `Observable { prn, pseudorange, doppler, cn0, timestamp_s }`
  - Добавлен контейнер эпохи наблюдаемых `ObservableSet`
    - фильтрация пригодных измерений (`is_usable`)
    - критерий навигационного решения: ≥ 4 спутников
  - Добавлены механизмы подготовки к валидации наблюдаемых:
    - интерфейсы для будущего сравнения с эталонными GNSS данными
      (RINEX-compatible design)
    - текущая валидация реализована через синтетические тестовые данные
  - Добавлена модульная структура:
    - `pseudorange.rs` — основная модель измерений
    - `doppler.rs` — доплер и псевдоскорость
    - `cn0.rs` — оценка качества сигнала и веса
    - `base.rs` — базовые типы и логика Observable
    - `mod.rs` — точка интеграции observable-слоя
...
- **navigation**
  - Добавлен `BitSynchronizer` для детекции границы навигационного бита по статистике
    BPSK-переходов на 20 возможных фазах.
  - Добавлен `BitAccumulator` для мажоритарного объединения 20 1-мс эпох в один
    навигационный бит после фазовой синхронизации.
  - Добавлен `FrameDecoder` для поиска TLM-преамбулы `0x8B`, верификации parity
    и сборки parity-подтверждённого subframe.
  - Добавлены разбор `HOW`-слова, проверка `TLM_PREAMBLE`, вычисление parity по
...
- **navigation**
  - Разделён DSP-слой и навигационный слой: `BitSynchronizer`/`BitAccumulator`
    работают с 1-мс эпохами, а `FrameDecoder` потребляет уже выровненные
    навигационные биты.
  - Уточнена модель поиска границы бита: истинная фаза теперь выбирается как минимум
    по накопленным внутренним BPSK-переходам.
  - Исправлена инициализация `BitAccumulator`: первое окно теперь всегда собирается
    полностью, независимо от `boundary_phase`.
  - Переведён `transition_counts` на `u64`, чтобы исключить переполнение при очень
    длинных сессиях без `reset()`.
  - Улучшена структура FSM декодера: поиск преамбулы, верификация выравнивания и
    сборка subframe стали явными состояниями.
- **examples**:
...
- Исправлен баг короткого первого окна в `BitAccumulator` при
  `boundary_phase != 0`.
- Исправлен риск переполнения `transition_counts` при длительной непрерывной работе
  синхронизатора.
- Исправлена обработка parity-ошибок: частичный retry и полный resync теперь разделены
  и ведут себя предсказуемо.
- Исправлена и проверена корректность parity-формул и инверсии информационных битов
  `Di = di ⊕ D*30`.
