//! DSP-модуль для генерации и обработки комплексных несущих сигналов.
//!
//! Содержит реализацию:
//!
//! - [`Nco`] — численно управляемый осциллятор (Numerically Controlled
//!   Oscillator)
//! - [`Mixer`] — частотный микшер (IQ mixer)
//! - утилиты для генерации и частотного сдвига сигналов
//!
//! ## Основные возможности
//!
//! - генерация комплексной экспоненты `exp(j·φ)`
//! - точное управление фазой и частотой
//! - фазово-непрерывное изменение частоты (без скачков)
//! - частотное смешивание (up/down conversion)
//!
//! ## Применение
//!
//! Модуль используется в DSP и GNSS-приёмниках для:
//!
//! - переноса сигнала в базовую полосу (downconversion)
//! - генерации локальной несущей
//! - компенсации доплеровского сдвига
//! - синтеза тестовых сигналов
//!
//! ## Особенности реализации
//!
//! - фаза нормализуется в диапазон `[0, 2π)`
//! - амплитуда всегда равна 1 (численная устойчивость)
//! - поддерживаются отрицательные частоты (вращение по часовой стрелке)
//! - состояние NCO сохраняется между вызовами
//!
//! ## Примечание
//!
//! Функция [`mix_shift`] создаёт временный NCO и **не сохраняет фазу**
//! между вызовами (каждый вызов начинается с φ = 0).

use std::f32::consts::TAU;

use num_complex::Complex32;

/// Численно управляемый осциллятор (Numerically Controlled Oscillator, NCO).
///
/// Генерирует комплексный сигнал вида `exp(j·φ)`, где фаза `φ` увеличивается
/// на фиксированный шаг `Δφ = 2π·f/f_s` на каждом сэмпле.
///
/// Используется для:
/// - генерации несущей
/// - частотного сдвига (IQ mixing)
/// - синтеза синусоидальных сигналов
#[derive(Debug, Clone)]
pub struct Nco {
    /// Текущая фаза (в радианах).
    phase: f32,

    /// Приращение фазы на один сэмпл (радианы/сэмпл).
    phase_step: f32,
}

/// Частотный микшер (frequency mixer / IQ mixer).
///
/// Выполняет умножение входного IQ-сигнала на комплексную экспоненту
/// `exp(j·2π·f·t)` для:
/// - сдвига спектра вверх (upconversion)
/// - сдвига спектра вниз (downconversion)
/// - переноса базовой полосы
///
/// Внутренне использует NCO, состояние которого сохраняется между вызовами.
#[derive(Debug, Clone)]
pub struct Mixer {
    /// Локальный генератор комплексной несущей.
    nco: Nco,

    /// Частота дискретизации сигнала (Hz).
    sample_rate_hz: f64,
}

impl Nco {
    /// Создаёт новый NCO с заданной частотой.
    ///
    /// # Аргументы
    /// - `freq_hz` — частота генерируемого сигнала (Hz)
    /// - `sample_rate_hz` — частота дискретизации (Hz)
    #[must_use]
    pub fn new(
        freq_hz: f64,
        sample_rate_hz: f64,
    ...
    /// Изменяет частоту генерации без сброса фазы.
    ///
    /// Это позволяет выполнять плавный frequency sweep без фазового скачка.
    ...
    /// Возвращает текущую фазу (радианы).
    #[inline]
    #[must_use]
    ...
    /// Возвращает приращение фазы на один сэмпл (радианы).
    #[inline]
    #[must_use]
    pub const fn phase_step_rad(&self) -> f32 {
        self.phase_step
    }

    /// Сбрасывает фазу в ноль.
    pub const fn reset(&mut self) {
        self.phase = 0.0;
    }

    /// Генерирует N комплексных отсчётов без внешнего сигнала.
    ///
    /// # Аргументы
...
impl Mixer {
    /// Создаёт новый частотный микшер.
    ///
    /// # Аргументы
    /// - `freq_hz` — частота смешивания (Hz)
    /// - `sample_rate_hz` — частота дискретизации (Hz)
    #[must_use]
    pub fn new(
        freq_hz: f64,
    ...
    /// Устанавливает частоту микширования.
    ///
    /// Фаза при этом не сбрасывается.
    pub fn set_frequency(
        &mut self,
        freq_hz: f64,
    ) {
    ...
    /// Изменяет частоту на заданное смещение (Hz).
    ///
    /// Используется для frequency tuning (например, ручная подстройка).
    ...
    /// Выполняет частотное смешивание (out-of-place).
    ///
    /// Каждый входной сэмпл умножается на `exp(j·φ[n])`.
    ///
    /// # Возвращает
    /// Новый вектор с результатом.
    pub fn mix(
        &mut self,
        input: &[Complex32],
    ) -> Vec<Complex32> {
    ...
    /// Выполняет частотное смешивание на месте (in-place).
    ///
    /// Модифицирует входной буфер без выделения памяти.
    pub fn mix_inplace(
        &mut self,
...
/// Быстрое частотное смещение сигнала.
///
/// Создаёт временный NCO и применяет смешивание.
#[must_use]
pub fn mix_shift(
    input: &[Complex32],
    freq_hz: f64,
...
/// Генерирует комплексную несущую.
///
/// # Аргументы
