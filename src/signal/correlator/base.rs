//! Базовый EPL-коррелятор.
//!
//! Содержит функцию [`correlator_epl`], выполняющую когерентную корреляцию
//! входного сигнала с тремя кодовыми репликами за один интервал интеграции.
//!
//! # Алгоритм
//!
//! Для каждого сэмпла `n`:
//!
//! ```text
//! E += s[n] * code_early[n]
//! P += s[n] * code_prompt[n]
...
//!
//! * `signal` — IQ-сэмплы **с удалённой несущей** (carrier wiped-off)
//! * `code_*` — кодовые реплики (обычно ±1.0 для BPSK)
//! * Длины могут различаться — итерация ограничена минимальной длиной
//!
//! # Применение
//!
//! ```text
//! RF → downconversion → [ correlator_epl ] → EplOutput → DLL / PLL
//! ```
...
/// # Аргументы
///
/// * `signal` — входной IQ-сигнал с удалённой несущей
/// * `code_early` — Early-реплика кода (опережение на ½ чипа)
/// * `code_prompt` — Prompt-реплика кода (текущая оценка фазы)
/// * `code_late` — Late-реплика кода (запаздывание на ½ чипа)
///
/// # Возвращает
///
/// [`EplOutput`] с комплексными накоплениями для каждой ветви.
#[must_use]
